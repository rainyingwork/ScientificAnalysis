...
class TagText () :
    ...
    def __init__(self):
        self.featureDict = {}
        self.featureDF = pandas.DataFrame()

    # ==================================================              ==================================================

    def makeFeatureDFByGeneralFD(self):
        tagShapeColumns = self.getTagShapeColumns()
        tagDictionaryArr = []
        for key in self.featureDict.keys():
            tagData = self.featureDict[key]
            entity = {}
            entity["common_011"] = tagData['enname']
            entity["common_012"] = tagData['cnname']
            entity["common_013"] = tagData['tagorder']
            entity["common_014"] = tagData['memo']
            entity["common_015"] = json.dumps(tagData['jsonmessage'], ensure_ascii=False)
            for count in range(0, len(tagData['tagshape'])):
                entity[tagShapeColumns[count]] = tagData['tagshape'][count]
            tagDictionaryArr.append(entity)
        self.featureDF = pandas.DataFrame(tagDictionaryArr)

    # def makeFeatureDFByDataToTagFD(self, featureName=[]):
    #     tagDictionarys = []
    #     for key in self.featureDict.keys():
    #         tagData = self.featureDict[key]
    #         tagDictionary = []
    ...
    #         for _ in range( 0 , len(tagData['tagshape'])):
    #             tagDictionary.append(tagData['tagshape'][_])
    ...
    #                      , "common_014"  # 標籤說明
    #                      , "common_015"  # 標籤JSON
    #                  ] + [col_name for col_name in featureName]
    #     self.featureDF = df
    #
    # def makeFeatureDFByTagToTagTextFD(self, featureName='common_001'):
    #     tagorder = 0
    #     tagDictionarys = []
    #     for key in self.featureDict.keys():
    #         tagData = self.featureDict[key]
    ...
    #     self.featureDF = df
    #
    # def makeFeatureDFByLevelToTagTextFD(self, featureName=["double_001", "double_002"]):
    #     tagDictionarys = []
    #     for key in self.featureDict.keys():
    #         tagData = self.featureDict[key]
    #         tagDictionary = []
    ...
    #         tagDictionary.append(tagData['memo'])
    #         tagDictionary.append(tagData['jsonmessage'])
    #         for _ in range(len(tagData['tagshape'])):
    #             tagDictionary.append(tagData['tagshape'][_])
    #         tagDictionarys.append(tagDictionary)
    #     df = pandas.DataFrame(tagDictionarys)
    #     df.columns = [
    ...
    def setFeatureDictByFilePath(self, filePath):
        tagJsonFilePath = f'{filePath}'
        with open(tagJsonFilePath, encoding='utf8') as file:
            dictData = json.load(file)
        self.featureDict = dictData
    ...
    def setFeatureDF(self, featureDF):
    ...
    def getCommonColumns(self):
        commonColumns = ["common_011", "common_012", "common_013", "common_014", "common_015"]
        return commonColumns

    def getTagShapeColumns(self):
        tagShapeColumns = [
        ...
        return tagShapeColumns

    def getJsonColumns(self):
        tagShapeColumns = ["json_001"]
        return tagShapeColumns
